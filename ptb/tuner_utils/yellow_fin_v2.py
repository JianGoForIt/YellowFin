...
class YFOptimizerUnit(object):
  def __init__(self, lr_val, mu_val, clip_thresh_val, alpha,
               high_pct=99.5, low_pct=0.5, gamma=0.9, 
               mu_update_interval=10, use_placeholder=False, use_async=False, slow_start_iters=200, use_cond_num_mom=True):
    ...
    if use_placeholder:
      ...
      # TODO reverse the variable setting for lr_var and mu_var
      self.lr_var = tf.placeholder(tf.float32, shape=() )
      self.mu_var = tf.placeholder(tf.float32, shape=() )
      ...
      # # TODO remove
      # self.lr_var = tf.Print(self.lr_var, [self.lr_var, ], message="lr value")
      # self.mu_var = tf.Print(self.mu_var, [self.mu_var, ], message="mu value")


    #     # DEBUG
    # print "smooth ", gamma
    # self.lr_var = tf.Print(self.lr_var, [self.lr_var, ], message="lr var init place")
    # self.mu_var = tf.Print(self.mu_var, [self.mu_var, ], message="mu var init place")
  ...
  def get_min_max_fisher_curvature(self):
    pass

  
  def get_fisher_mu_tensor(self):
    '''
    Fisher matrix based momentum estimation
    '''
    start_tune = tf.greater(self._global_step, 0)
  ...
  def get_mu_tensor(self):
    ...
    mu_sync = ( (tf.sqrt(dr) - 1) / (tf.sqrt(dr) + 1) )**2

    # mu_sync = tf.Print(mu_sync, [mu_sync, ], message="mu_sync")
  ...
  def apply_gradients(self, grads_tvars, staleness):
    self._grads, self._tvars = zip(*grads_tvars)
    self.setup_slots()

    # TODO remove simulation part
    self._grads = list(self._grads)
    self._grads[0] = tf.Print(self._grads[0], [self._grads[0], ], message="no delay grad")
    misc_ops = []
    print "inside optimizer simulating staleness ", staleness
    staleness = 2 + staleness
    self.stale_grads = []
    for i in range(staleness):
      if i == 0:
        self.stale_grads.append( [tf.add(g, 0.0) for g in self._grads] )
      else:
        self.stale_grads.append( [tf.Variable(tf.zeros(t.get_shape().as_list() ), trainable=False) for t in self._grads] )

    with tf.control_dependencies(self.stale_grads[0] ):
      pop_ops = []
      for i in range(staleness - 1):
        if i == 0:
          pop_op = [tf.assign(g_delay, g) \
            for g_delay, g in zip(self.stale_grads[staleness - 1 - i], 
            self.stale_grads[staleness - 2 - i] ) ]
        else:
          with tf.control_dependencies(pop_op):
            pop_op = [tf.assign(g_delay, g) \
              for g_delay, g in zip(self.stale_grads[staleness - 1 - i], 
              self.stale_grads[staleness - 2 - i] ) ]
        pop_ops += pop_op
      misc_ops = tf.group(*pop_ops)
      self._grads_delay = self.stale_grads[-1]
      self._grads_delay[0] = tf.Print(self._grads_delay[0], [self._grads_delay[0], ], message="delayed grad")
    # END of TODO
    ...
    with tf.variable_scope("apply_updates"):
      with tf.control_dependencies( [before_apply_op] ):
        self._grads_clip, self._grads_norm = tf.clip_by_global_norm(self._grads_delay, self.clip_thresh_var)
        apply_grad_op = \
    ...
    #   # start_tune = tf.greater(self._global_step, 10)
    #   # lr = tf.cond(start_tune, lambda: self.get_lr_tensor(), lambda: tf.constant(0.0, tf.float32) )
    #   # mu_sync = tf.cond(start_tune, lambda: self.get_mu_tensor(), lambda: tf.constant(0.0, tf.float32) )

    #   lr = self.get_lr_tensor()
    #   with tf.control_dependencies( [lr] ):
    ...
    #   if self.use_async:
    #     # mu_async = tf.cond(start_tune, lambda: self.get_async_mu_tensor(), lambda: mu_sync) 
    #     mu_async = self.get_async_mu_tensor()
        
    #     # for DEBUG
    #     self.mu_async_test = mu_async
    #     ####

    # # #     # # DEBUG
    # # #     # mu_async = tf.Print(mu_async, [mu_async, ], message="mu async")
    # # #     # mu_sync = tf.Print(mu_sync, [mu_sync, ], message="mu sync")
    # # #     self._mu_async = mu_async
    # # #     self._mu_sync = mu_sync
    #     with tf.control_dependencies( [mu_sync, mu_async] ):
    #       assign_mu_op = tf.assign(self.mu_var, self.mu_var + mu_sync - mu_async)
    # # #     # assign_mu_op = tf.assign(self.mu_var, mu_sync)
    # # #     # assign_mu_op = tf.assign(self.mu_var, tf.constant(0.8, tf.float32) )
    # # #     assign_mu_op = tf.no_op()
    #   else:
    #     with tf.control_dependencies( [mu_sync] ):
    #       assign_mu_op = tf.assign(self.mu_var, mu_sync)

    # with tf.control_dependencies( [assign_lr_op, assign_mu_op] ):
    ...
    return tf.group(before_apply_op, apply_grad_op, after_apply_op, misc_ops)
    # return tf.group(before_apply_op, apply_grad_op, after_apply_op, 
    #   assign_lr_op, assign_mu_op, misc_ops, self._increment_global_step_op)
...
class MetaOptimizer(object):
  def __init__(self, lr_vals, mu_vals, clip_thresh_vals, 
               ...
               gamma=0.9, use_hyper_op=False, use_async=False, slow_start_iters=200):
    ...
    self._optimizers = [YFOptimizerUnit(lr_val, mu_val, clip_thresh_val,
                        ...
                        alpha=alpha/np.sqrt(float(len(lr_vals) ) ), 
                        # TODO better logic to specify placeholder or not
                        gamma=gamma,
                        use_placeholder=False,
                        use_async=use_async,
                        slow_start_iters=slow_start_iters) \
  ...
  def apply_gradients(self, grad_tvar_list, staleness):
    '''
    ...
    assert len(grad_tvar_list) == len(self._optimizers)
    assert len(grad_tvar_list) == len(self._optimizers)
    self._apply_grad_ops = [opt.apply_gradients(grad_tvar, staleness) \
