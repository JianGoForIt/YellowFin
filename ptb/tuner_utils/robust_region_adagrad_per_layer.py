...
class OptimizerUnit(object):
  ...
  # def on_iter_finish(self, grad_vals, use_hyper_op=False):
  #   # get gradient norm and projected gradient norm

  #   self.local_curv_list = []
  ...
  #   self.local_curv_list.append(np.sum(val_list**2) )

  #   i = len(self.local_curv_list)
  #   w = i**(-1.0)
  ...
  #           get_lr_and_mu_rave_noisy(local_curv_list, dist_star_estimate**2, grad_var2_list[-1] * 0.01, T, min_curv, max_curv)                
...
class MetaOptimizer(object):
  ...
  def on_iter_finish(self, grad_vals_list, loss):
    lr_vals = []
    mu_vals = []
    clip_thresh_vals = []
    hyper_ops = []
    ...
    for optimizer, grad_vals in zip(self._optimizers, grad_vals_list):
      if use_hyper_op:
        hyper_op = optimizer.on_iter_finish(grad_vals, use_hyper_op)
        hyper_ops.append(hyper_op)
      else:
        optimizer.on_iter_finish(grad_vals, use_hyper_op)
