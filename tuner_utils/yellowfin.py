...
class YFOptimizer(object):
  ...
  def curvature_range(self):
    ...
    with tf.control_dependencies([self._h_min_t, self._h_max_t] ):
      ...
      with tf.control_dependencies([avg_op] ):
        self._h_min = tf.exp(tf.identity(self._moving_averager.average(self._h_min_t) ) ) * 0.74
        self._h_max = tf.exp(tf.identity(self._moving_averager.average(self._h_max_t) ) ) * 0.74
  ...
  def grad_variance(self):
    ...
    self._grad_var = tf.maximum(tf.constant(1e-6, dtype=self._grad_norm_squared_avg.dtype), 
      ...
      - tf.add_n( [tf.reduce_sum(val) for val in self._grad_avg_squared] ) ) * 0.74
    return grad_var_ops
  ...
  def dist_to_opt(self):
    ...
    dist_to_opt_ops.append(avg_op)
    with tf.control_dependencies([avg_op]):
      self._dist_to_opt_avg = tf.identity(self._moving_averager.average(self._dist_to_opt) ) / tf.sqrt(0.74)
